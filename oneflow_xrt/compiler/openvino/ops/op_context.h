...
class OpenvinoOpContext : public OpContext {
 public:
  struct Param {
    ...
    // Input operands
    std::map<Argument, std::shared_ptr<ngraph::Node>> inputs;
    int input_size;
  ...
  explicit OpenvinoOpContext(
      const Param& param, const std::map<Argument, Parameter>& entry_params_map)
  ...
  const std::map<Argument, std::shared_ptr<ngraph::Node>>& graph_inputs()
      const {
    return graph_inputs_;
  }
  const std::map<Argument, std::shared_ptr<ngraph::Node>>& graph_weight()
      const {
    return graph_weight_;
  }
  // Return output as OpenvinoValues
  const std::map<Argument, std::shared_ptr<ngraph::Node>>& outputs() const {
    return outputs_;
  }
 ...
 private:
  ...
  Param param_;
  // Output operands
  std::map<Argument, std::shared_ptr<ngraph::Node>> graph_inputs_;
  std::map<Argument, std::shared_ptr<ngraph::Node>> graph_weight_;
  std::map<Argument, std::shared_ptr<ngraph::Node>> outputs_;

  const std::map<Argument, Parameter>& entry_params_map_;
};
