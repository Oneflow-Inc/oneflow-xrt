...
inline ngraph::element::Type DataTypeToNgraphDataType(
    const DataType& data_type) {
  switch (data_type) {
    case oneflow::kDouble:
      return ngraph::element::f64;
    case oneflow::kFloat:
...
class NgraphShape {
 public:
  ...
  NgraphShape(const Shape& shape, const DataType& data_type)
      : shape_(ShapeToNgraphShape(shape)),
        data_type_(DataTypeToNgraphDataType(data_type)) {}
