...
class BroadcastDivGradOp : public TrtOpKernel {
 public:
  void Compile(TrtOpContext* ctx) override {
    // r0 = z / y
    auto* r0 = ctx->builder()->addElementWise(
        *(ctx->Input("z_0")), *(ctx->Input("y_0")),
        nvinfer1::ElementWiseOperation::kDIV);
    std::string r0_name = ctx->op_name() + ".div";
    r0->setName(r0_name.c_str());
    // r1 = dz * r0
    auto* r1 = ctx->builder()->addElementWise(
        *(ctx->Input("dz_0")), *(r0->getOutput(0)),
        nvinfer1::ElementWiseOperation::kPROD);
    std::string r1_name = ctx->op_name() + ".mul";
    r0->setName(r1_name.c_str());
    // r3 = -r1
    auto* r3 = ctx->builder()->addUnary(*(r1->getOutput(0)),
    ...
    for (int i = reduced_shape.NumAxes() - 1; i >= 0; --i) {
      if (large_shape.At(i) != reduced_shape.At(i)) {
        reduce_axes |= (1U << i);
      }
    }
    auto* reduce = ctx->builder()->addReduce(*(r3->getOutput(0)),
    ...
    ctx->SetOutput("dy_0",
                   helpers::Reshape(ctx, reduce->getOutput(0), reduced_shape));
  }
};

REGISTER_TRT_OP_KERNEL(broadcast_div_grad, BroadcastDivGradOp)
    .EnableTrainPhase()
    .Finalize();
