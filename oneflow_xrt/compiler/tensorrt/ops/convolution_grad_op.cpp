...
class ConvDataGradOp : public TrtOpKernel {
 public:
  void Compile(TrtOpContext* ctx) override {
    ...
    nvinfer1::Weights bias{nvinfer1::DataType::kFLOAT /* type */,
                           nullptr /* values */, 0 /* count */};
    auto* deconv = ctx->builder()->addDeconvolution(
        *input, filters, nvinfer1::DimsHW(kernel_size[0], kernel_size[1]),
        weight, bias);
    ...
    if (!helpers::DimsEqual(size, output->getDimensions())) {
      auto* slice_output =
          ctx->builder()->addSlice(*output, start, size, stride);
      ...
      auto* identity =
          ctx->builder()->addIdentity(*(slice_output->getOutput(0)));
...
REGISTER_TRT_OP_KERNEL(conv_data_grad, ConvDataGradOp).Finalize();

}  // namespace tensorrt
}  // namespace xrt
}  // namespace oneflow

#endif  // NV_TENSORRT_VERSION >= 8200
