...
// #include "oneflow_xrt/compiler/tensorrt/plugin/prelu_grad_plugin.h"
 ...
 public:
  void Compile(TrtOpContext* ctx) override {
    ...
    auto* ones = ctx->builder()->addConstant(
        ShapeToXrtDims(shape),
        helpers::Constant(ctx, 1.f, shape, data_type, ones_name));
    ones->setName(ones_name.c_str());
    // 1 - alpha
    auto* sub_alpha = ctx->builder()->addElementWise(
        *(ones->getOutput(0)), *alpha, nvinfer1::ElementWiseOperation::kSUB);
    std::string sub_alpha_name = ctx->op_name() + ".sub_alpha";
    sub_alpha->setName(sub_alpha_name.c_str());
    // clip(x, 0, 1)
    auto* clip =
        ctx->builder()->addActivation(*x, nvinfer1::ActivationType::kCLIP);
    clip->setAlpha(0.f);
    clip->setBeta(1.f);
    std::string clip_name = ctx->op_name() + ".clip";
    clip->setName(clip_name.c_str());
    // ceil(clip(x, 0, 1))
    auto* ceil = ctx->builder()->addUnary(*(clip->getOutput(0)),
                                          nvinfer1::UnaryOperation::kCEIL);
    std::string ceil_name = ctx->op_name() + ".ceil";
    ceil->setName(ceil_name.c_str());
    // (1 - alpha) * ceil(clip(x, 0, 1))
    auto* mul0 = ctx->builder()->addElementWise(
        *(sub_alpha->getOutput(0)), *(ceil->getOutput(0)),
        nvinfer1::ElementWiseOperation::kPROD);
    std::string mul0_name = ctx->op_name() + ".mul0";
    mul0->setName(mul0_name.c_str());
    // alpha + (1 - alpha) * ceil(clip(x, 0, 1))
    auto* add = ctx->builder()->addElementWise(
        *alpha, *(mul0->getOutput(0)), nvinfer1::ElementWiseOperation::kSUM);
    std::string add_name = ctx->op_name() + ".add";
    add->setName(add_name.c_str());
    // dy * (alpha + (1 - alpha) * ceil(clip(x, 0, 1)))
    auto* mul1 = ctx->builder()->addElementWise(
        *dy, *(add->getOutput(0)), nvinfer1::ElementWiseOperation::kPROD);
    ...
    if (alpha_requires_grad) {
      ...
      auto* sub = ctx->builder()->addElementWise(
          *(ones->getOutput(0)), *(ceil->getOutput(0)),
          nvinfer1::ElementWiseOperation::kSUB);
      std::string sub_name = ctx->op_name() + ".alpha_diff.sub";
      sub->setName(sub_name.c_str());
      auto* mul2 = ctx->builder()->addElementWise(
          *x, *(sub->getOutput(0)), nvinfer1::ElementWiseOperation::kPROD);
      std::string mul2_name = ctx->op_name() + ".alpha_diff.mul2";
      mul2->setName(mul2_name.c_str());
      auto* mul3 = ctx->builder()->addElementWise(
          *dy, *(mul2->getOutput(0)), nvinfer1::ElementWiseOperation::kPROD);
      ...
      auto* reduce = ctx->builder()->addReduce(*(mul3->getOutput(0)),
                                               nvinfer1::ReduceOperation::kSUM,
                                               reduce_axes, /*keepdims*/ false);
...
REGISTER_TRT_OP_KERNEL(prelu_grad, PReluGradOp).EnableTrainPhase().Finalize();
