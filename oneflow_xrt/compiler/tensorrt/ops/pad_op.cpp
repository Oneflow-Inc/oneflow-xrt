...
class PaddingOp : public TrtOpKernel {
 public:
  void Compile(TrtOpContext* ctx) override {
    ...
    if (padding_before[0] != 0 || padding_before[1] != 0 ||
        padding_after[0] != 0 || padding_after[1] != 0) {
      UNIMPLEMENTED()
          << "TensorRT does not support padding batch and channel dimension";
    }
...
REGISTER_TRT_OP_KERNEL(pad, PaddingOp).EnableTrainPhase().Finalize();
