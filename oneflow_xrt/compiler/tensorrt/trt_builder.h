...
class TrtBuilder {
 private:
  ...
  std::map<int64_t, TrtValueKind> value_kinds_;
  std::map<int64_t, Parameter> params_;
  std::map<int64_t, nvinfer1::ITensor*> tensors_;
  std::map<int64_t, nvinfer1::Weights> weights_;

  std::map<std::string, std::shared_ptr<std::vector<uint8_t>>> host_weights_;

 public:
  explicit TrtBuilder(const std::string& name)
      : builder_name_(name), next_handle_(-1) {
