...
*/
#include "oneflow_xrt/compiler/xla/ops/optimizer_op.h"
...
class AdamOptimizerOp : public OptimizerOp {
 private:
  void ApplyUpdate(XlaOpContext* ctx, xla::XlaOp gradient,
    ...
    ctx->SetOutput("m_0", m);
    ctx->SetOutput("v_0", v);
    ctx->SetOutput("model_0", weight - lr * (m / (xla::Sqrt(v) + epsilon) +
                                             weight_decay * weight));
    if (do_bias_correction) {
...
REGISTER_XLA_OP_KERNEL(adam_update, AdamOptimizerOp)
    .MarkModelUpdateOp()
