...
#include "oneflow_xrt/compiler/xla/xla_helpers.h"
...
void LayerNormOp::Compile(XlaOpContext* ctx) {
  ...
  if (ctx->HasOutput("mean_0")) {
    data_type = ctx->OutputType("mean_0");
  }
  ...
  if (ctx->HasOutput("mean_0")) {
    ctx->SetOutput("mean_0", Reshape(mean, mean_shape));
  }
...
void LayerNormParamGradOp::Compile(XlaOpContext* ctx) {
  ...
  if (ctx->HasOutput("gamma_diff_0")) {
    ...
    xla::XlaOp gamma_grad =
        xla::Mul(xla::Sub(x, mean, batch_dims /*broadcast dim*/), inv_variance,
...
REGISTER_XLA_OP_KERNEL(layer_norm, LayerNormOp).Finalize();
REGISTER_XLA_OP_KERNEL(layer_norm_param_grad, LayerNormParamGradOp).Finalize();
REGISTER_XLA_OP_KERNEL(layer_norm_grad, LayerNormGradOp).Finalize();
